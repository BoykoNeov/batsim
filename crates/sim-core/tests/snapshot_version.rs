...
//!
//! v10 is different. Adding `CellModel::Spm` to an externally-tagged enum does not
//! change how the existing variants serialize, and the chemistry's `[spm]` section is
//! optional, so a v9 blob is **still structurally valid** at v10 — it would restore
//! into a working pack whose semantics this build no longer guarantees.
//! `Pack::restore`'s version check is the only thing standing there.
//!
//! # The fixture is an equivalent-circuit pack, deliberately
//! Not an SPM one. The hazard is a snapshot a **v9 build could actually have
//! written**, and no v9 build could write an SPM pack — the variant did not exist.
//! An all-ECM pack's v10 bytes are, field for field, what v9 emitted, so retagging
//! them to 9 produces a genuine article rather than a v10 blob wearing a v9 label.
//!
...
/// A v9-tagged snapshot is rejected by the version check, and the **same bytes**
/// tagged v10 restore.
///
...
#[test]
fn the_version_field_is_what_rejects_a_v9_snapshot() {
    assert_eq!(
        SNAPSHOT_VERSION, 10,
    ...
    assert_eq!(
        ...
        "a v9-tagged snapshot must be refused"
    );
...
fn only_the_outer_version_tag_is_consulted() {
    let bytes = snapshot_bytes();
    // Both blobs carry an inner `Pack.version` of 10, because that is what this
    // build wrote. Only the outer tag differs, and only the outer tag decides.
    assert!(Pack::restore(&retagged(&bytes, 9)).is_err());
