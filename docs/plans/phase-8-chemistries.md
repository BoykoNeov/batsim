...
measurement and wins**. (Slices 0, A, B and C have since landed and the repo is at
...
| **D** | **Teach NiMH,** including the falling end-of-charge voltage. Carries exit criterion 4. Its shape is already decided by the spike and slice C: a **-ΔV in millivolts on a telemetry row**, with an isothermal control arm beside it — **not** a peak on a plot, because the taper is cut and the corner is a one-timestep artifact. The estimator bias hysteresis creates (up to 25 mV, invisible to `ocv_invert` by design) is a second contrast available for free. | v18 (no bump) |
