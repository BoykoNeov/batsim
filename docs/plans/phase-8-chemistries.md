...
**Status: COMPLETE, 2026-08-28. Authored as pre-work, and nothing below was measured when
...
| **2. Both new chemistries are taught — CLOSED 2026-08-28 (slices B and D).** Each has a parameter file with provenance on every constant, a scenario file, guided-path steps, and claims in `path-claims.toml` under the digits rule. LTO is steps 25–26; NiMH is steps 27–29. | slices B, D |
| **3. The floor did not move — CLOSED 2026-08-28 (slice C), structurally.** Every existing trajectory — ECM, SPM, DFN, lead-acid — is bit-identical before and after the phase, unless a slice argues a measured exception the way slice A of Phase 7 did. A new file that no existing pack loads cannot move one; ~~the hysteresis state in slice C is the place this criterion is actually at risk~~ **(it was not: both v18 terms are `Option`-gated *paths* rather than neutral zeros — `ecm_overpotential_v` and `advance_cell` never execute the hysteresis term, and `open_circuit_v` returns the bare table lookup — so no chemistry without the sections can move by a ULP, by construction. Pinned by `no_chemistry_but_nimh_carries_the_v18_sections`, which is what guards the half construction cannot: a future edit adding a section to a shipped file.)** | slice C, re-checked by every slice |
| **4. The nickel cell's end-of-charge signature is emergent — CLOSED 2026-08-28 (slice C measured it, slice D made it reachable).** The voltage of a full NiMH cell on constant current **falls**, and that fall must come out of the physics — the charge the cell stops accepting, the heat that makes, and ~~a negative `docv_dt_v_per_k`~~ **(AMENDED 2026-08-28: that third item works *backwards* as the engine stands. `docv_dt` reaches voltage only as heat, and on charge it cools the cell — arm C of the spike cooled 4 K below ambient before the clamp. Slice D must be measured against a `docv_dt` that is read into OCV, per the spike's recommendation, or against `R0(T)` alone — not against this sentence as originally written.)** — not from a scripted override. `CLAUDE.md` forbids the scripted kind, and this is the phase's one genuinely new emergent behaviour. ~~**Whether the existing overcharge-heat path (`energy-hole.md`) is enough to produce it is the spike question and is not known today.**~~ **ANSWERED 2026-08-28: not on its own.** The fall emerges, but through `R0(T)` alone at roughly half the size needed at the instant a charger detects it, with a one-timestep corner. See `phase-8-slice-c-spike.md`. | slice C spike (**done**), then slice D |
...
| **A** | **LANDED 2026-08-27** — `docs/plans/phase-8-slice-a-lto.md`. `chemistries/lto_20ah_generic.toml`, datasheet-anchored on a 20 Ah LTO cell. **Zero engine code changed, so exit criterion 1 closes on the zero-code branch** — with one caveat found by reading rather than running: `[safety]` had **no way to say a cell does not plate**, because zeroing the cost fields still raises the flag and dropping the section switches off runaway too, so the file shipped a labelled sentinel. **That caveat was fixed after the phase closed** — the gate is an optional pair as of `SNAPSHOT_VERSION` 19 and the sentinel is gone; see `docs/plans/plating-absence.md`. Two *test-side* guards had to be amended, and both had assumed every cell is a graphite cell. | v17 (no bump) |
...
| **C** | **LANDED 2026-08-28** — `docs/plans/phase-8-slice-c-hysteresis.md`. The `[hysteresis]` state, the `ocv.t_ref_k` temperature correction, and `chemistries/nimh_subc_3ah_generic.toml`. The spike's recommendation taken whole, **including its cut** — no charge-acceptance taper. Exit criterion 3 closes **structurally on both halves**, so the criterion was never at risk here despite this table saying it would be. Found by measuring rather than reading: the first version of the -ΔV experiment charged from 90 %, which is barely one time constant of the cell's slow RC pair, and that pair was still filling through the overcharge and **lifting** the voltage by 3.3 mV against the fall — a measurement taken before the transients it is not about had settled. Also found: a **non-lithium** chemistry can say "does not plate" cleanly by dropping `[safety]`, which narrows the schema gap slice A recorded. | **v17 → v18** |
| **D** | **LANDED 2026-08-28 — `docs/plans/phase-8-slice-d-nimh-client.md`. THE PHASE CLOSES HERE.** Four scenario files, guided-path steps 27–29, twenty-four claims, two arms, eight ledger rules. No engine code. The predicted shape held — a **-ΔV in millivolts on a telemetry row** with an isothermal control arm, and the estimator bias taken as the free second contrast — but the **-ΔV needed THREE steps rather than two**, and that was measured rather than chosen: only a step's mark ever pauses this page, so a fall between two instants needs **two marks on one continuous run**, and the `run-on` arm both this table and the advisor proposed is unwalkable by a reader. Also found: the drafted sentence about the `heat` row was false (that row is **negative** for most of a NiMH charge — the cell cools before it warms), and the double-cover panic caught three over-general rules in a row. | v18 (no bump) |
