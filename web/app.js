...
const LESSONS = [
  ...
  },
  {
    ...
    prose: [
      ...
      "This runs to 200 000 s of simulation — about 56 hours — which is twenty seconds of watching at 10 000×. Then raise the ambient slider to 45 °C and press Run.",
    ],
    expect:
  ...
  },
  {
    ...
    expect:
      "93 amps on the first frame after the fault, and **73.5 seconds of no flags at all**. By the time the BMS says anything the current has settled to 86 and the groups have sagged to 2.13 V — above `v_min` itself, so not even a soft under-voltage — while the charge trace falls off a cliff. The first thing the BMS says is `OT` at t = 133.5 s, by which point the pack is already at 51 %. It latches at **t = 156.0 s**, 96 s after the short, at **39.62 %**: against the twin's 0.56 points, this fault costs **fifty points**, because the rung that caught the bigger short was never in this one's way. Two details worth pausing on. The trip is a *probe* crossing 343.15 K — the two probes sit on corner cells, and the cell that is genuinely hottest is at 344.52 K when it fires, so protection is late by 1.3 K of somebody else's temperature. And the pack peaks at 344.5 K here, against a twin whose run ends at 299.1 K: the weaker short is the more dangerous one, which is not the intuition. Now uncheck the BMS and run it again — and then keep running, because this step's mark is 200 s and everything worth seeing on the unprotected arm happens after it. Press Run at the mark and let it run on until the current is down to hundredths of an amp. The trajectory is identical — the same 93.29 A on the first frame after the fault, the same 87.02 A thirty seconds later — right up to the instant it isn't, and then it simply carries on past the mark: `SOC_CLAMPED_LOW` at 235.5 s, at 375.6 K and still rising, with 58 A flowing. What ends this run is not protection and not the fault being removed. It is the pack running out. Past the clamp the cells keep supplying the short, but their open-circuit voltage is now falling — 58.04 A at the flag, 40.33 A four and a half seconds later, 17.03 A at 250 s, 1.94 A at 300 s, 0.103 A at 400 s — so the short goes on being connected and stops mattering. The temperature turns round with it: it peaks at **376.4 K at 245.5 s**, ten seconds after the flag, and is back to 362 K by 400 s. Nobody intervened. A dead short across an unprotected pack ends when the pack is flat, and the last thing you see is the `past empty` readout climbing while everything else falls.",
  },
  {
    id: "past-empty",
